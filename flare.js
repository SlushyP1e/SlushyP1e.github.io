...
(function() {
    ...
    // Respect users who prefer less motion — skip cursor, tilt and particles.
    ...
    // Runs a bind function whenever elements are added to the page,
    // debounced to one call per frame (replaces setInterval polling).
    const watchForNewElements = (bindFn) => {
        let scheduled = false;
        const obs = new MutationObserver(() => {
            if (scheduled) return;
            scheduled = true;
            requestAnimationFrame(() => { scheduled = false; bindFn(); });
        });
    ...
    const injectSharedDOMElements = () => {
        ...
        if (!isTouch && !prefersReducedMotion) {
            ...
            addHoverStates();
            // Rebind when new elements appear instead of polling every second
            watchForNewElements(addHoverStates);
        }
    ...
    document.addEventListener('click', (e) => {
        playSound('click');
        if (!prefersReducedMotion) spawnClickParticles(e.clientX, e.clientY);
    });
